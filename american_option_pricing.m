...
% The following plots illustrate this data. Specifically, the plots show 
% the relative price movements of each index and the Euribor risk-free rate proxy. 
% The initial level of each index has been normalized to unity to 
% facilitate the comparison of relative performance over the historical record
...
% Plotting the Risk Free Rate (3-Month Euribor Continuously-Compounded)
...
% Fitting a Pareto Tail Curve In for every country's index and plotting
% for a particular country (France, DAX)
...
% Creating the Pereto tails with smoothened Gaussian Kernel and EVT (Extreme
% Value Theory applied to 10% of each tail)
...
% Plotting a Semi-Parametric/Peicewise distribution as an Example
...
% Copula Calibration
% When the uniform variates are transformed by the empirical CDF of each 
% margin, the calibration method is often known as canonical maximum 
% likelihood (CML). The following code segment first transforms the daily 
% centered returns to uniform variates by the piecewise, semi-parametric 
% CDFs derived above. It then fits the Gaussian and t copulas to the transformed data:
...
% Copula Simulation
% Now that the copula parameters have been estimated, we simulate 
% jointly-dependent uniform variates using the function copularnd.
...
% We now simulate and plot centered returns using the Gaussian 
...
% American Option Pricing Using Markov Chain Monte Carlo Simulation
